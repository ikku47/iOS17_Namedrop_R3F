...
extend({
  TextureTransformMaterial,
  BlurDownSampleMaterial,
  BlurUpSampleMaterial,
  WaveMaterial,
  ParticleMaterial,
...
declare global {
  namespace JSX {
    interface IntrinsicElements {
      textureTransformMaterial: React.DetailedHTMLProps<
        React.HTMLAttributes<ShaderMaterial>,
        ShaderMaterial
      >;
      blurDownSampleMaterial: React.DetailedHTMLProps<
        React.HTMLAttributes<ShaderMaterial>,
        ShaderMaterial
      >;
      blurUpSampleMaterial: React.DetailedHTMLProps<
        React.HTMLAttributes<ShaderMaterial>,
        ShaderMaterial
      >;
      waveMaterial: React.DetailedHTMLProps<
        React.HTMLAttributes<ShaderMaterial>,
        ShaderMaterial
      >;
      particleMaterial: React.DetailedHTMLProps<
        React.HTMLAttributes<ShaderMaterial>,
        ShaderMaterial
      >;
      proceduralLightMaterial: React.DetailedHTMLProps<
        React.HTMLAttributes<ShaderMaterial>,
        ShaderMaterial
      >;
    }
...
const Interface = ({ isTriggered }: InterfaceProps) => {
  ...
  const { scale_transform, translation_transform, stretch_y_factor } =
    useControls("Transform", {
      scale_transform: {
        ...
        onChange: (v) => {
          if (textureTransformMaterialRef.current) {
            textureTransformMaterialRef.current.uniforms.scale_transform.value =
              1 / v;
          }
        },
      },
      ...
      stretch_y_factor: {
        ...
        onChange: (v) => {
          if (textureTransformMaterialRef.current) {
            textureTransformMaterialRef.current.uniforms.stretch_y_factor.value =
              v;
          }
        },
      },
      ...
      translation_transform: {
        ...
        onChange: (v) => {
          if (textureTransformMaterialRef.current) {
            textureTransformMaterialRef.current.uniforms.translation_transform.value =
              v;
          }
        },
      },
      ...
      translation_transform: THREE.Vector2;
      stretch_y_factor: number;
    };
  ...
  useEffect(() => {
    if (kawaseBlurMaterialRefA.current) {
      kawaseBlurMaterialRefA.current.uniforms.resolution.value =
        new THREE.Vector2(size.width, size.height);
    }
    if (kawaseBlurMaterialRefB.current) {
      kawaseBlurMaterialRefB.current.uniforms.resolution.value =
        new THREE.Vector2(size.width, size.height);
    }
    if (kawaseBlurMaterialRefC.current) {
      kawaseBlurMaterialRefC.current.uniforms.resolution.value =
        new THREE.Vector2(size.width, size.height);
    }
    if (kawaseBlurMaterialRefD.current) {
      kawaseBlurMaterialRefD.current.uniforms.resolution.value =
        new THREE.Vector2(size.width, size.height);
    }

    if (waveMaterialRef.current) {
      waveMaterialRef.current.uniforms.resolution.value = new THREE.Vector2(
        size.width,
        size.height
      );
    }
    ...
    if (particleMaterialRef.current) {
      particleMaterialRef.current.uniforms.resolution.value = new THREE.Vector2(
        size.width,
        size.height
      );
    }
    ...
    if (proceduralLightMaterialRef.current) {
      proceduralLightMaterialRef.current.uniforms.resolution.value =
        new THREE.Vector2(size.width, size.height);
    }
  }, []);
  ...
  useFrame(({ clock }) => {
    ...
    // DownSample - Pass 1
    if (kawaseBlurMaterialRefA.current) {
      kawaseBlurMaterialRefA.current.uniforms.buff_tex.value =
        textureTransformFBO.texture;
    ...
    // DownSample - Pass 2
    if (kawaseBlurMaterialRefB.current) {
      kawaseBlurMaterialRefB.current.uniforms.buff_tex.value =
        kawaseBlurFBOA.texture;
    ...
    // UpSample - Pass 3
    if (kawaseBlurMaterialRefC.current) {
      kawaseBlurMaterialRefC.current.uniforms.buff_tex.value =
        kawaseBlurFBOB.texture;
    ...
    // UpSample - Pass 4
    if (kawaseBlurMaterialRefD.current) {
      kawaseBlurMaterialRefD.current.uniforms.buff_tex.value =
        kawaseBlurFBOC.texture;
    ...
    if (proceduralLightMaterialRef.current) {
      proceduralLightMaterialRef.current.uniforms.buff_tex.value =
        waveFBO.texture; //lightFBO.texture
  ...
  const [lightFade, setLightFade] = useSpring(() => ({
    ...
    onChange: (v) => {
      if (proceduralLightMaterialRef.current) {
        proceduralLightMaterialRef.current.uniforms.light_mix_factor.value =
          v.value.lightMixFactor;
      }
    },
  ...
  const [lightPropsGo, springApiLightGo] = useSpring(
    () => ({
      ...
      onChange: (v) => {
        if (proceduralLightMaterialRef.current) {
          proceduralLightMaterialRef.current.uniforms.light_center.value =
            v.value.lightCenter;
          proceduralLightMaterialRef.current.uniforms.top_light_strength.value =
            v.value.topLightStrength;
          proceduralLightMaterialRef.current.uniforms.light_expotential_factor.value =
            v.value.lightExpFactor;
  ...
  const [blurFade, setBlurFade] = useSpring(() => ({
    ...
    onChange: (v) => {
      if (
        kawaseBlurMaterialRefA.current &&
        kawaseBlurMaterialRefB.current &&
        kawaseBlurMaterialRefC.current &&
        kawaseBlurMaterialRefD.current
      ) {
        kawaseBlurMaterialRefA.current.uniforms.blurOffset.value =
          v.value.blurOffset;
        kawaseBlurMaterialRefB.current.uniforms.blurOffset.value =
          v.value.blurOffset;
        kawaseBlurMaterialRefC.current.uniforms.blurOffset.value =
          v.value.blurOffset;
        kawaseBlurMaterialRefD.current.uniforms.blurOffset.value =
          v.value.blurOffset;
      }
    },
  }));
  ...
  const [pulseProps, springApiPulse] = useSpring(
    () => ({
      ...
      onChange: (v) => {
        if (particleMaterialRef.current) {
          particleMaterialRef.current.uniforms.pusleFactor.value =
            v.value.particlePulseFactor;
          particleMaterialRef.current.uniforms.particle_amount.value =
            v.value.particle_amount;
        }

        if (waveMaterialRef.current) {
          waveMaterialRef.current.uniforms.waveFactor.value =
            v.value.wavePulseFactor;
        }
      },
